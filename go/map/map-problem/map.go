...
func main() {
	...
	for _, v := range elements {
		m1[RandStringBytesMaskImprSrc(30)] = v
	}
	...
	for {
		i++
		for _, v := range m1 {
			m2[RandStringBytesMaskImprSrc(30)] = v
		}
		for k, _ := range m2 {
			delete(m2, k)
		}
		...
		stats = new(runtime.MemStats)
		runtime.ReadMemStats(stats)
		fmt.Printf("Alloc After M2 %d次:\t\t%5.2fMB - delta:%5.2fMB\n", i, ByteSize(stats.Alloc)/MB, ByteSize(stats.Alloc-previous)/MB)
		previous = stats.Alloc
	}
}
